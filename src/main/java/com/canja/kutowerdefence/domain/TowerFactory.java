...
public final class TowerFactory {
    ...
    public static Tower createTower(MapObjectType towerType, Point location, GameSession gameSession) {
        return switch (towerType) {
            case TOWER_ARCHER -> new ArcherTower(location, gameSession, range[ARCHER_INDEX], damage[ARCHER_INDEX]);
            case TOWER_ARTILLERY -> new ArtilleryTower(location, gameSession, range[ARTILLERY_INDEX], damage[ARTILLERY_INDEX], aoeRadius);
            case TOWER_MAGE -> new MageTower(location, gameSession, range[MAGE_INDEX], damage[MAGE_INDEX]);
            default -> null;
        };
    }
}
